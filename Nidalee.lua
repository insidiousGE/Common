...
OnLoop(function(myHero)
        ...
        if IOW:Mode() == "Combo" then
                ...
                local QPred = GetPredictionForPlayer(GoS:myHeroPos(),target,GetMoveSpeed(target),1350,250,1500,37.5,true,false)

                        if CanUseSpell(myHero, _Q) == READY and QPred.HitChance == 1 and GoS:ValidTarget(target, 2000) and GoS:GetDistance(myHero, target) <= 1400 and NidaleeMenu.Combo.Q:Value() then
                    ...
                    for _, ally in pairs(GoS:GetAllyHeroes()) do    
                        ...
                        if CanUseSpell(myHero, _E) == READY and NidaleeMenu.Combo.AE:Value() and GoS:ValidTarget(target, 1500) and GoS:GetDistance(myHero, ally) <= 600 and (GetCurrentHP(ally)/GetMaxHP(ally))<0.5 then
...
if NidaleeMenu.Drawings.Q:Value() then DrawCircle(GoS:myHeroPos().x, GoS:myHeroPos().y, GoS:myHeroPos().z,1400,3,100,0xff00ff00) end
